...
class OgreMaterialGenerator(object):
    ...
    def generate(self):
        ...
        with self.w.iword('material').word(self.material_name).embed():
            if self.material.shadow_method is not 'NONE':
    ...
    def generate_pass( self, mat, pass_name="" ):
        ...
        with self.w.embed():
            ...
            self.w.iline('ambient %s %s %s %s' %(color[0], color[1], color[2], alpha) )
            ...
            self.w.iline('specular %s %s %s %s %s' %(s.r*f, s.g*f, s.b*f, alpha, mat.specular_intensity/4.0) )

            #f = mat.emit
            #if mat.use_vertex_color_light:
            #    self.w.iline('emissive vertexcolour' )
            #else:
            self.w.iline('emissive %s %s %s %s' %(color[0], color[1], color[2], alpha) )
            self.w.iline('')
